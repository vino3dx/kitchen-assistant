...
/** 查找某位成员是否已点指定菜品。 */
export async function findOrder(recipeId: string, userName: string): Promise<Order | null> {
  const { data, error } = await supabaseClient.from("orders").select("*").eq("recipe_id", recipeId).eq("user_name", userName).maybeSingle();
  if (error) throw error;
