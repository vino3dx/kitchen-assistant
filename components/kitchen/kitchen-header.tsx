...
/** 厨房应用页头及三阶段导航。 */
export function KitchenHeader({ activeTab, onTabChange, orderCount, shopping, cooking }: KitchenHeaderProps) {
  ...
  return <header id="kitchen-header" className="sticky top-0 z-30 bg-white/95 backdrop-blur-md border-b border-stone-200 shadow-xs">
    <div className="max-w-4xl mx-auto px-4 py-3 flex items-center justify-between"><div className="flex items-center gap-2.5"><div className="w-10 h-10 rounded-xl bg-gradient-to-tr from-amber-500 to-orange-500 text-white flex items-center justify-center shadow-xs"><ChefHat className="w-6 h-6" /></div><div><div className="flex items-center gap-2"><h1 className="text-lg font-bold tracking-tight text-stone-900" style={{ width: "73.9375px" }}>厨房助手</h1><span className="text-[11px] px-2 py-0.5 rounded-md bg-amber-100 text-amber-800 font-medium border border-amber-200" style={{ width: "61px" }}>Kitchen Assistant</span></div><p className="text-xs text-stone-500">点菜 → 智能买菜清单 → 烹饪指南</p></div></div><Link id="btn-nav-admin" href="/admin" className="inline-flex items-center gap-1.5 px-3 py-1.5 rounded-lg text-xs font-medium text-stone-700 bg-stone-100 hover:bg-stone-200 transition-colors"><BookOpen className="w-3.5 h-3.5" /><span>菜谱库管理</span></Link></div>
